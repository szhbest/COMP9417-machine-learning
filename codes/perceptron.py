...
def use_cv(k_dict):
    length = len(k_dict)
    for a in [0.0001, 0.0002, 0.0003, 0.0004, 0.0005, 0.0006, 0.0007, 0.0008, 0.0009, 0.0010]:
        test_accuracy = 0
        train_accuracy = 0
        ...
        for k in k_dict:
            ...
            # Perceptron model
            clf = Perceptron(penalty='l2', max_iter=1000, tol=1e-3, alpha=a, random_state=0).fit(X_train, y_train)

            test_accuracy += compute_acc(clf, X_test, y_test)
            train_accuracy += compute_acc(clf, X_train, y_train)

        print(f"{a}: train accuracy: {train_accuracy / length}, test accuracy: {test_accuracy / length}")
